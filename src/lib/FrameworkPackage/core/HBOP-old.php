...
function loadModule($argHint, $argClassExistsCalled = FALSE){
	...
	static $autoGenerateFlag = NULL;

	$corefilename = strtoupper(substr(basename(__FILE__), 0, strpos(basename(__FILE__), '.')));
	$hintPath = str_replace('/', '.', $argHint);

	if(NULL === $autoGenerateFlag){
		$autoGenerateEnabled = dirname(dirname(__FILE__)).'/.autogenerate';
		if(defined($corefilename . '_AUTO_GENERATE_ENABLED')){
			$autoGenerateEnabled = constant($corefilename . '_AUTO_GENERATE_ENABLED');
		}
		// 自動ジェネレートフラグのセット
		$autoGenerateFlag = file_exists($autoGenerateEnabled);
	}
	...
	if(TRUE === $autoGenerateFlag){
		...
		$generatedIncFileName = dirname(dirname(__FILE__)).'/autogenerate/'.$hintPath.'.generated.inc.php';
		...
		if(isset($pkConfXML->{$packageName}->instance)){
			...
			if(!class_exists($instanceName, FALSE)){
				if(FALSE === $argClassExistsCalled){
					_systemError('not found class ' . $instanceName . ' on ' . $pkConfXML->{$packageName}->link[$packagePathCnt] . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
				}else{
					return FALSE;
				}
			}
...
function _loadDefaultModule($argPkConfXMLs, $argPackageName, $argHint, $argClassExistsCalled = FALSE, $argFileGetContentsEnabled = FALSE){
	...
	$matches = NULL;
	if(preg_match('/^controlmain\.(.+)/', $argPackageName, $matches)){
		...
		for($packageCnt = 0; count($argPkConfXMLs) > $packageCnt; $packageCnt++){
			...
			if(isset($argPkConfXML->default) && isset($argPkConfXML->default->controlmain)){
				for($packagePathCnt = 0, $errorCnt = 0; count($argPkConfXML->default->controlmain) > $packagePathCnt; $packagePathCnt++){
					if(TRUE === $argFileGetContentsEnabled){
						$file = @file_get_contents($argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName.$argPkConfXML->default->controlmain[$packagePathCnt]->attributes()->suffix, TRUE);
						if(strlen($file) > 0){
							$path = $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName.$argPkConfXML->default->controlmain[$packagePathCnt]->attributes()->suffix;
							$loaded = TRUE;
							BREAK 2;
						}
						$file = @file_get_contents($argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName, TRUE);
						if(strlen($file) > 0){
							$path = $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName;
							$loaded = TRUE;
							BREAK 2;
						}
					}else{
						if(FALSE !== @include_once($argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName.$argPkConfXML->default->controlmain[$packagePathCnt]->attributes()->suffix)){
							$loaded = TRUE;
							// ジェネレート処理
							if(TRUE === $autoGenerateFlag){
								$generatedIncFileName = dirname(dirname(__FILE__)).'/autogenerate/'.$hintPath.'.generated.inc.php';
								// 先ず先頭に条件文を追加
								@file_put_contents_e($generatedIncFileName, '<?php' . sprintf($autoGenerateIncPHPMainBase, $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName.$argPkConfXML->default->controlmain[$packagePathCnt]->attributes()->suffix) . '?>', FILE_PREPEND);
								// ファイルの終端に処理を追加
								@file_put_contents_e($generatedIncFileName, '<?php if(FALSE === $unlink){ @include_once(\'' . $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName.$argPkConfXML->default->controlmain[$packagePathCnt]->attributes()->suffix . '\'); } ?>', FILE_APPEND);
								@chmod($generatedIncFileName,0777);
							}
							BREAK 2;
						}
						if(FALSE !== @include_once($argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName)){
							$loaded = TRUE;
							// ジェネレート処理
							if(TRUE === $autoGenerateFlag){
								$generatedIncFileName = dirname(dirname(__FILE__)).'/autogenerate/'.$hintPath.'.generated.inc.php';
								// 先ず先頭に条件文を追加
								@file_put_contents_e($generatedIncFileName, '<?php' . sprintf($autoGenerateIncPHPMainBase, $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName) . '?>', FILE_PREPEND);
								// ファイルの終端に処理を追加
								@file_put_contents_e($generatedIncFileName, '<?php if(FALSE === $unlink){ @include_once(\'' . $argPkConfXML->default->controlmain[$packagePathCnt].'/'.$argPackageName . '\'); } ?>', FILE_APPEND);
								@chmod($generatedIncFileName,0777);
							}
							BREAK 2;
						}
		...
		if(FALSE === $loaded){
			if(FALSE === $argClassExistsCalled){
				_systemError('not found controlmain ' . $argPackageName . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
			}else{
				return FALSE;
			}
		}
		...
		if(preg_match('/^modelmain\.(.+)/',$argPackageName,$matches)){
			...
			if(FALSE === $loaded){
				if(FALSE === $argClassExistsCalled){
					_systemError('not found modelmain ' . $argPackageName . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
				}else{
					return FALSE;
				}
			}
			...
			if(preg_match('/^consolemain\.(.+)/',$argPackageName,$matches)){
				...
				if(FALSE === $loaded){
					if(FALSE === $argClassExistsCalled){
						_systemError('not found console  ' . $argPackageName . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
					}else{
						return FALSE;
					}
				}
				...
				if(preg_match('/^abstract\.(.+)/',$argPackageName,$matches)){
					...
					if(FALSE === $loaded){
						if(FALSE === $argClassExistsCalled){
							_systemError('not found abstract ' . $argPackageName . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
						}else{
							return FALSE;
						}
					}
					...
					if(preg_match('/^interface\.(.+)/',$argPackageName,$matches)){
						...
						if(FALSE === $loaded){
							if(FALSE === $argClassExistsCalled){
								_systemError('not found interface ' . $argPackageName . '!! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
							}else{
								return FALSE;
							}
						}
...
function _initFramework($argment = NULL){
	...
	if(NULL === $pkgConfXML){
		...
		$configPaths = Configure::constant('.*PACKAGE_CONFIG_XML_PATH.*', TRUE);
		...
		if(NULL === $pkgConfXML){
			_systemError('not found package.xml ' . $pkgConfXMLPath. ' !! Please check default path config.' . "\n" . str_replace(PATH_SEPARATOR, "\n", get_include_path()));
		}
	}
	// $pkgConfXMLのリフレッシュ処理
 ...
 */
function loadConfig($argConfigPath, $isVendorConfig=FALSE){
	...
	if(TRUE === $autoGenerateFlag){
		if(is_file($argConfigPath)){
			...
			if(TRUE !== $regenerateFlag){
				...
				$generatedConfigFileName = dirname(dirname(__FILE__)).'/autogenerate/'.$configFileName.'.generated.php';
...
if(TRUE === class_exists('Configure', FALSE)){
	$paths = Configure::constant(".+_PATH$",TRUE);
	foreach($paths as $key => $val){
		set_include_path(get_include_path().PATH_SEPARATOR.$val);
	}
}
